		...
		if (!empty($menus))
		{
			...
			if (empty($level))
			{
				...
				foreach ($menus as $k => $menu)
				{
					...
					if (!empty($sub))
					{
						$out .= '<li class="dropdown"><a role="button" data-toggle="dropdown" tabindex="-1" href="'.menu_link($menu['link']).'" title="'.$menu['title'].'">'.$menu['title'].' <b class="caret"></b></a>'.$sub.'</li>';
					}else{
						$act = @$_GET['menu_id']==$menu['id'] ? ' class="active"' : '';
						$out.= '<li'.$act.'><a href="'.menu_link($menu['link']).'" title="'.$menu['title'].'">'.$menu['title'].'</a></li>';
					}
				}
				...
				foreach ($menus as $k => $menu)
				{
					...
					if (!empty($sub))
					{
						$out .= '<li class="dropdown-submenu"><a tabindex="-1" href="'.menu_link($menu['link']).'" title="'.$menu['title'].'">'.$menu['title'].'</a>'.$sub.'</li>';
					}else{
						$act = @$_GET['menu_id']==$menu['id'] ? ' class="active"' : '';
						$out.= '<li'.$act.'><a href="'.menu_link($menu['link']).'" title="'.$menu['title'].'">'.$menu['title'].'</a></li>';
					}
